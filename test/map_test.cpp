...
void	eraseTest( void ) {
	...
	for ( ; it4 != mp2.end(); it4 = mp2.begin() ) {
		std::cout << it4->first << std::endl; mp2.erase( it4->first ); 
	}
...
std::vector<int> upper_bound_test(std::map<int, int> mp) {
    std::vector<int> v;
    ...
    mp.insert(std::make_pair(-10, 10));
    mp.insert(std::make_pair(-20, 20));
    v.push_back((--mp.lower_bound(0))->first);
    mp3.lower_bound(490000);
    return v;
}

std::vector<int> upper_bound_test(ft::map<int, int> mp) {
    std::vector<int> v;
...
void	boundTest( void ) {
	...
	std::vector<int>	v1 = upper_bound_test( mp );
	std::vector<int>	v2 = upper_bound_test( mpo );

	std::vector<int>::iterator	itv1, itv2;

	for ( itv1 = v1.begin(), itv2 = v2.begin(); itv2 != v2.end(); ++itv1, ++itv2 ) {
		std::cout << "Origin: " << *itv2 << " \t" << *itv1 << std::endl;
		// if ( *itv1 != *itv2 )
		// 	std::cout << "Error: " << *itv1 << " " << *itv2 << std::endl;
	}
}
